...
import { Search, ShoppingCart, MapPin, Loader2, X, CreditCard, CheckCircle } from 'lucide-react';
...
interface FormattedProduct {
  id: string;           // SellerInventory.id — cart key
...
/** Jaipur-area test locations that actually match seeded shop coordinates. */
...
/** Categories derived from BigBasket seed data. */
const CATEGORIES = [
  'Fruits & Vegetables',
  'Dairy & Eggs',
  'Bakery',
...
export const BuyerDashboard: React.FC = () => {
  ...
  // --- filter state ---
  const [searchInput, setSearchInput]       = useState('');   // raw input value (shown in box)
  ...
  // --- active test location label for button highlight ---
  ...
  // Derived flag — single source of truth for "lock everything" state
  const isProcessing = checkoutState === 'loading';

  // Auto-reset error banner after 5 s so the UI never hangs on a failure state
  ...
  // -------------------------------------------------------------------------
  // Fetch products whenever location, debounced query, or category changes
  // -------------------------------------------------------------------------
  useEffect(() => {
    if (!location) return;
    ...
    fetch(`${API_BASE}/api/products/search?${params.toString()}`)
      ...
      .then(data => {
        setProducts(
          data.map(p => ({
            id:             String(p.id),
            productCatalogId: String(p.product_catalog_id),
            shopId:         String(p.shop.id),
            name:           p.name,
  ...
  const handleCheckout = () => {
    startCheckout({
      ...
      onLoadingEnd: () => {
        setCheckoutState(prev => (prev === 'loading' ? 'idle' : prev));
      },
      onSuccess: (paymentId) => {
        setCheckoutState('success');
        setCheckoutMsg(`Payment confirmed! ID: ${paymentId}`);
      },
      onFailure: (reason) => {
        if (reason === 'Payment cancelled.') {
          ...
          setCheckoutMsg('Payment was unsuccessful. Please try again.');
          scheduleErrorReset();
        }
      },
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 flex items-center gap-1 text-sm">
                ...
                {location
                  ? `Showing results near ${activeLocation ?? 'your location'}`
                  : 'Locating…'}
              </p>
            </div>
            ...
            <div className="relative w-full md:w-96">
              ...
              <input
                ...
                className="block w-full pl-10 pr-9 py-3 border border-gray-200 rounded-xl leading-5 bg-white
                           placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-primary-500
                           focus:border-primary-500 sm:text-sm shadow-sm transition-all duration-200"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
          {loading ? (
            ...
            <div className="col-span-full text-center py-12 text-gray-500">
              Select a test location above or allow browser location access to see nearby products.
            </div>
          ) : products.length === 0 ? (
            <div className="col-span-full text-center py-12 text-gray-500">
            ...
            products.map(product => (
              <ProductCard key={product.id} product={product} disabled={isProcessing} />
            ))
          )}
        </div>
      </main>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-8 h-8 rounded-md border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      ...
                      <button
                        ...
                        disabled={isProcessing}
                        className="text-xs font-semibold text-red-600 hover:text-red-700 hover:underline disabled:opacity-40 disabled:cursor-not-allowed disabled:no-underline"
                      >
                        Remove
                      </button>
                    </div>
            ...
            <div className="pt-4 border-t border-gray-200 mt-4">
              ...
              {checkoutState === 'error' && checkoutMsg && (
                <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      onClick={() => { setCheckoutState('idle'); setCheckoutMsg(null); }}
              ...
              <button
                ...
                disabled={checkoutState === 'loading' || cart.length === 0}
      ...
      ================================================================ */}
      <button
        className="lg:hidden fixed bottom-6 right-6 bg-primary-600 text-white p-4 rounded-full shadow-lg hover:bg-primary-700 transition-colors z-20"
